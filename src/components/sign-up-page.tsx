...
const SignUpPage = ({ isDonationPage }: { isDonationPage?: boolean }) => {
  ...
  const { data: links, isLoading: isLinksLoading } =
    useGetProfileLinks(address);
  const { data: profile, isLoading: isProfileLoading } =
    useGetProfileByAddress(address);

  const isButtonDisabled = isConnected && !!address && !token;
  ...
  useEffect(() => {
    if (address && isConnected && links && profile) {
      // If they already have a profile created, redirect them back to their profile
      const hasProfileLink = links.find((link) => link.type === "profile");
      if (
        hasProfileLink &&
        !window.location.href.includes("create/fundraiser")
      ) {
        navigate(`/profile/${profile.user.userName}`);
      }
    }
  }, [address, isConnected, isLinksLoading, links, isProfileLoading]);
  ...
  if (isLinksLoading || isProfileLoading) {
    return <LoadingPage message="We are preparing to create your link..." />;
  }
